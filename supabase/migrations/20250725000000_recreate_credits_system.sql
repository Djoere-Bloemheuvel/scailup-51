-- Recreate Credits System - Lovable Compatible
-- This migration recreates the entire credits system with a main "credits" table
-- while maintaining compatibility with existing code and Lovable
...
-- 12. Create function to check and use credits (for backward compatibility)
...
BEGIN
  -- Find admin client
  SELECT id INTO admin_client_id FROM clients WHERE email = 'djoere@scailup.io' LIMIT 1;
  
  IF admin_client_id IS NOT NULL THEN
    -- Set unlimited credits for all modules and credit types
    FOR module_record IN SELECT id, slug FROM modules LOOP
      CASE module_record.slug
        WHEN 'lead_engine' THEN
          PERFORM set_unlimited_credits(admin_client_id, module_record.id, 'leads');
          PERFORM set_unlimited_credits(admin_client_id, module_record.id, 'contacts');
          PERFORM set_unlimited_credits(admin_client_id, module_record.id, 'enrichment');
        WHEN 'marketing_engine' THEN
        ...
        WHEN 'analytics' THEN
          PERFORM set_unlimited_credits(admin_client_id, module_record.id, 'reports');
          PERFORM set_unlimited_credits(admin_client_id, module_record.id, 'insights');
        ELSE
          PERFORM set_unlimited_credits(admin_client_id, module_record.id, 'standard');
      END CASE;
    END LOOP;
  END IF;
END $$;

-- 18. Create trigger to automatically add credits for new clients
...
INSERT INTO admin_logs (
  ...
  'credits_system_recreated',
  '{"action": "recreated_entire_credits_system", "main_table": "credits", "backward_compatible": true, "lovable_compatible": true}',
  NOW()
); 