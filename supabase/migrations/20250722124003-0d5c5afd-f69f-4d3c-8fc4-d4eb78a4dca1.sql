...
-- Remove client_id column from leads table and update related policies
-- This will make leads accessible to all clients

-- First, drop all existing RLS policies that depend on client_id
...
-- Create new RLS policies that allow all authenticated users to access leads
...
-- Update the upsert_lead_with_enrichment function to not use client_id
CREATE OR REPLACE FUNCTION public.upsert_lead_with_enrichment(
  ...
  p_client_id UUID DEFAULT NULL -- Keep parameter for backwards compatibility but ignore it
)
...
-- Update get_current_client_id function to handle the new structure
...
-- Update get_lead_status_counts function to work without client filtering
...
BEGIN
  ...
  -- Count total leads with valid emails (now global, not per client)
