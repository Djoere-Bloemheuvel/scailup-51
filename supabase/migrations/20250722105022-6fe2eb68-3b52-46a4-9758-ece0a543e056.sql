-- Update leads table to match contacts table structure and add missing components
-- This ensures consistency between leads and contacts tables

-- First add missing columns to leads table to match contacts structure
ALTER TABLE public.leads 
ADD COLUMN IF NOT EXISTS client_id UUID REFERENCES public.clients(id) ON DELETE CASCADE,
ADD COLUMN IF NOT EXISTS status TEXT DEFAULT 'new',
DROP COLUMN IF EXISTS user_id; -- Remove user_id as we're using client_id instead

-- Update leads table structure to be consistent
ALTER TABLE public.leads 
...
-- Update leads RLS policy to work with client_id instead of user_id
...
-- Add more missing columns to match expected structure
