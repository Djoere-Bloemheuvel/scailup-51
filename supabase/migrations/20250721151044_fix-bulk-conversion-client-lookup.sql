-- Fix bulk conversion client lookup with better error handling
-- Update the bulk_convert_leads_to_contacts function to handle client lookup more robustly
...
BEGIN
  ...
  -- Try to get client_id using multiple methods
  -- Method 1: Try users.client_id relationship
  SELECT c.id INTO current_client_id
  FROM clients c
  JOIN users u ON c.id = u.client_id
  WHERE u.id = auth.uid();
  
  -- Method 2: If that fails, try to find client by email
  IF current_client_id IS NULL THEN
    SELECT id INTO current_client_id
    FROM clients
    WHERE email = current_user_email;
  END IF;
  
  -- Method 3: If still no client, try to find any client (fallback for testing)
  IF current_client_id IS NULL THEN
    SELECT id INTO current_client_id
    FROM clients
    LIMIT 1;
  ...
  IF current_client_id IS NULL THEN
    RAISE EXCEPTION 'Client not found for user. User email: %, auth.uid(): %', current_user_email, auth.uid();
  END IF;
...
'Bulk convert multiple leads to contacts with robust client lookup and clean table structure'; 