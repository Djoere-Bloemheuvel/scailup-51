-- Refresh schema cache and ensure functions are properly registered
-- This script will force Supabase to recognize the functions

-- 1. Drop and recreate the functions to refresh schema cache
...
-- 2. Recreate convert_lead_to_contact function
...
-- 4. Recreate check_and_use_credits function
...
-- 5. Create bulk_convert_leads_to_contacts function
...
-- 6. Force schema cache refresh by calling the functions
SELECT 'Schema cache refresh completed' as status;

-- 7. Test the functions
SELECT 'Testing convert_lead_to_contact function' as test_name;
-- This will fail with expected error, but ensures the function is registered
SELECT convert_lead_to_contact('00000000-0000-0000-0000-000000000000'::UUID, 'Test');

SELECT 'Testing get_contacts_with_lead_data function' as test_name;
SELECT COUNT(*) as contacts_count FROM get_contacts_with_lead_data();

SELECT 'Testing bulk_convert_leads_to_contacts function' as test_name;
SELECT COUNT(*) as bulk_results FROM bulk_convert_leads_to_contacts(ARRAY['00000000-0000-0000-0000-000000000000'::UUID], 'Test'); 